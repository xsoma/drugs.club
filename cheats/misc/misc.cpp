...
void misc::watermark()
{
	...
	auto watermark = VERSION + g_ctx.pizdadaaaaaaddadada + crypt_str("") + g_ctx.globals.time;
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = VERSION + g_ctx.pizdadaaaaaaddadada + crypt_str("") + server + crypt_str(" |") + crypt_str(" ping: ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms ");
	...
	//text
	render::get().text(fonts[NAME], width - 10 - box_width + 5, 21, Color(255, 255, 255, 220), HFONT_CENTERED_Y, watermark.c_str());
