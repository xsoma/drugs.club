...
void eventlogs::paint_traverse()
{
	...
	for (size_t i = 0; i < logs.size(); i++)
	{
		...
		last_y -= 14;
		auto text_width = render::get().text_width(fonts[LOGS], log.message.c_str() );
	
		auto text_height = render::get().text_height(fonts[LOGS], log.message.c_str() );
		auto logs_size_inverted = 10 - logs.size();
		render::get().gradient(log.x-5, last_y + log.y - logs_size_inverted * 14 , text_width +10, text_height+3 , Color(g_cfg.misc.logscolorl.r(), g_cfg.misc.logscolorl.g(), g_cfg.misc.logscolorl.b(), g_cfg.misc.logscolorl.a()), Color(g_cfg.misc.logscolorr.r(), g_cfg.misc.logscolorr.g(), g_cfg.misc.logscolorr.b(), g_cfg.misc.logscolorr.a()), GRADIENT_HORIZONTAL); //GRADIENT_HORIZONTAL

		render::get().text(fonts[LOGS], log.x, last_y + log.y - logs_size_inverted * 14, log.color, HFONT_CENTERED_NONE, log.message.c_str()); //HFONT_CENTERED_NONE
...
void eventlogs::add(std::string text, bool full_display)
{
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CONSOLE])
	{
		...
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ flipix ] "));
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
	{
		...
		auto log = crypt_str("[ \x0Cflipix \x01] ") + text;
