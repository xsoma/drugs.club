...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	auto factor = (spawn_time + inferno_t::get_expiry_time() - m_globals()->m_curtime) / inferno_t::get_expiry_time();
		...
		render::get().Draw3DFilledCircle(origininf, 150, g_cfg.esp.grenade_3dfire_color); //Draw3DFilledCircle
	...
	render::get().rect_filled(screen_origin_m.x - size_m.x * 0.5f, screen_origin_m.y - size_m.y * 0.5f - 1.0f, size_m.x, size_m.y, Color(37, 37, 37, g_cfg.esp.molotov_timer_color.a()));
	render::get().rect_filled(screen_origin_m.x - size_m.x * 0.5f + 2.0f, screen_origin_m.y - size_m.y * 0.5f, (size_m.x - 4.0f) * factor, size_m.y - 2.0f, g_cfg.esp.molotov_timer_color);

	render::get().rect(screen_origin_m.x - size_m.x * 0.5f, screen_origin_m.y - size_m.y * 0.5f, size_m.x, size_m.y, Color(7, 7, 7, g_cfg.esp.molotov_timer_color.a()));
	render::get().text(fonts[ESP], screen_origin_m.x, screen_origin_m.y - size_m.y * 0.5f + 12.0f, g_cfg.esp.molotov_timer_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "FIRE");
	render::get().text(fonts[GRENADES], screen_origin_m.x + 1.0f, screen_origin_m.y - size_m.y * 0.5f - 9.0f, g_cfg.esp.molotov_timer_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "l");



		

	//		render::get().Draw3DFilledCircle(origininf, 150, g_cfg.esp.grenade_3dfire_color_enemy);
		...
		//	render::get().Draw3DFilledCircle(origin, 150, g_cfg.esp.grenade_3dfire_color_enemy);
	
			//render::get().Draw3DFilledCircle(origin, 150, g_cfg.esp.grenade_3dfire_color);
...
void worldesp::smoke_timer(entity_t* entity)
{
	...
	auto origin_smoke = smoke->GetAbsOrigin();

	Vector screen_origin_smoke;

	if (!math::world_to_screen(origin_smoke, screen_origin_smoke))
		return;
	render::get().Draw3DFilledCircle(origin_smoke, 150, g_cfg.esp.grenade_3dsmoke_color);
	...
	static auto size_smoke = Vector2D(35.0f, 5.0f);
	render::get().circle_filled(screen_origin_smoke.x, screen_origin_smoke.y - size_smoke.y * 0.5f, 60, 20, Color(15, 15, 15, 187));

	render::get().rect_filled(screen_origin_smoke.x - size_smoke.x * 0.5f, screen_origin_smoke.y - size_smoke.y * 0.5f - 1.0f, size_smoke.x, size_smoke.y, Color(37, 37, 37, g_cfg.esp.smoke_timer_color.a()));
	render::get().rect_filled(screen_origin_smoke.x - size_smoke.x * 0.5f + 2.0f, screen_origin_smoke.y - size_smoke.y * 0.5f, (size_smoke.x - 4.0f) * factor, size_smoke.y - 2.0f, g_cfg.esp.smoke_timer_color);

	render::get().rect(screen_origin_smoke.x - size_smoke.x * 0.5f, screen_origin_smoke.y - size_smoke.y * 0.5f, size_smoke.x, size_smoke.y, Color(7, 7, 7, g_cfg.esp.smoke_timer_color.a()));
	render::get().text(fonts[ESP], screen_origin_smoke.x, screen_origin_smoke.y - size_smoke.y * 0.5f + 12.0f, g_cfg.esp.smoke_timer_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "SMOKE");
	render::get().text(fonts[GRENADES], screen_origin_smoke.x + 1.0f, screen_origin_smoke.y - size_smoke.y * 0.5f - 9.0f, g_cfg.esp.smoke_timer_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "k");
