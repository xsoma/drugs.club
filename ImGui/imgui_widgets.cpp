...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x , label_size.y + style.FramePadding.y * 2.0f); //+ style.FramePadding.x * 2.0f   // + style.FramePadding.y * 2.0f
    ...
    // Render
    const ImVec4 text_dis = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    const ImVec4 text_act = ImVec4(120 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
    const ImVec4 text_hov = ImVec4(120 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(label_size + ImVec2(1,30) , label_size.x *4.0f , label_size.y * 2.0f ); //size_arg
    
    const ImRect bb(pos, pos + size);
    ItemSize(size, style.FramePadding.x);
    if (!ItemAdd(bb, id))
        return false;
    ...
    // Render
    const ImVec4 text_dis = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    const ImVec4 text_act = ImVec4(112 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);

    const ImVec4 icon_dis = ImVec4(13 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(112 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
    ...
    PushFont(icon_font);
    RenderTextClipped(bb.Min - ImVec2(size_arg.x , 10 * c_menu::get().dpi_scale), bb.Max, icon, NULL, &icon_size, ImVec2(0.f, 0.27f), &bb);
    PopFont();
    float rot = 0.0001;
    ...
    for (int i = 0; i < 10; i++)
    {
     ...
     //   window->DrawList->AddLine(bb.Min + ImVec2(0, 50), bb.Min + ImVec2(115, 50), cRainbow, 2); //+ ImVec2(80, 60)
       

    }
   // window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), GetColorU32(ImVec4(44 / 255.f, 161 / 255.f, 246 / 255.f, 255 / 255.f)), GetColorU32(ImVec4(44 / 255.f, 161 / 255.f, 246 / 255.f, 255 / 255.f)), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)));


    PopStyleColor();
    const int style_alpha8 = IM_F32_TO_INT8_SAT(style.Alpha);
    const ImU32 col_white = IM_COL32(160, 160, 160, style_alpha8);
    PushStyleColor(ImGuiCol_Text, it_text->second);
    ...
    RenderTextClipped(bb.Min + ImVec2( 3, 0), bb.Max + ImVec2(0, 30), label, NULL, &label_size, ImVec2(0.f, 0.41f), &bb); //+ ImVec2(size_arg.x * 0.33f, (4 * c_menu::get().dpi_scale) - 4)
    PopStyleColor();
...
bool ImGui::BeginMenuBar()
{
    ...
    window->DrawList->AddRectFilledMultiColor(ImVec2(clip_rect.Min.x/2, clip_rect.Min.y/2),ImVec2(clip_rect.Max.x/2,clip_rect.Max.y/2), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 255 / 255.f)), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 255 / 255.f)), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)));
    window->DrawList->AddRectFilledMultiColor(ImVec2(clip_rect.Max.x/2, clip_rect.Min.y/2),ImVec2(clip_rect.Max.x, clip_rect.Max.y/2), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 255 / 255.f)), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 255 / 255.f)));
...
{
    ImGuiContext& g = *GImGui;
    PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(50, 50)); // ImGuiStyleVar_Alpha ImGuiStyleVar_FramePadding
    g.NextWindowData.MenuBarOffsetMinVal = ImVec2(10, ImMax(g.Style.DisplaySafeAreaPadding.y - g.Style.FramePadding.y, 0.0f));
    SetNextWindowPos(ImVec2(0.0f, 0.0f));
    

    SetNextWindowSize(ImVec2(g.IO.DisplaySize.x, g.NextWindowData.MenuBarOffsetMinVal.y + g.FontBaseSize + g.Style.FramePadding.y)); //g.IO.DisplaySize.x
    
    PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
    PushStyleVar(ImGuiStyleVar_WindowMinSize, ImVec2(150, 0)); //ImGuiStyleVar_WindowMinSize

   
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoScrollWithMouse;
    bool is_open = Begin("##MainMenuBar", NULL, window_flags) && BeginMenuBar();

    PopStyleVar(2);
    g.NextWindowData.MenuBarOffsetMinVal = ImVec2(150.0f, 0.0f);
  
   // RenderText(ImVec2(15, 5), "DRUGS");
    if (!is_open)
    {
        End();
        return false;
