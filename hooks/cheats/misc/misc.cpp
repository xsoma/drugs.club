...
#define VERSION crypt_str("flipix")
...
void misc::watermark()
{
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = VERSION + g_ctx.pizdadaaaaaaddadada + crypt_str(" | ") + server + crypt_str(" | ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + tickrate + crypt_str(" tick | ") + g_ctx.globals.time;
		}
	}
	...
	if (g_cfg.misc.verticalgrad)
	{

		render::get().gradient(width - 10 - box_width, 11, box_width, 18, Color(g_cfg.misc.misccolorl.r(), g_cfg.misc.misccolorl.g(), g_cfg.misc.misccolorl.b(), g_cfg.misc.misccolorl.a()), Color(g_cfg.misc.misccolorr.r(), g_cfg.misc.misccolorr.g(), g_cfg.misc.misccolorr.b(), g_cfg.misc.misccolorr.a()), GRADIENT_VERTICAL); //GRADIENT_HORIZONTAL

	}
	if (!g_cfg.misc.verticalgrad)
	{
		render::get().gradient(width - 10 - box_width, 11, box_width, 18, Color(g_cfg.misc.misccolorl.r(), g_cfg.misc.misccolorl.g(), g_cfg.misc.misccolorl.b(), g_cfg.misc.misccolorl.a()), Color(g_cfg.misc.misccolorr.r(), g_cfg.misc.misccolorr.g(), g_cfg.misc.misccolorr.b(), g_cfg.misc.misccolorr.a()), GRADIENT_HORIZONTAL); //GRADIENT_HORIZONTAL
	}
	...
	//render::get().rect_filled(width - 10 - box_width, 11, box_width, 18, Color(46, 46, 46, (int)g_cfg.misc.alpha));

	render::get().text(fonts[NAME], width - 10 - box_width + 5, 20, Color(g_cfg.misc.misctext.r(), g_cfg.misc.misctext.g(), g_cfg.misc.misctext.b(), g_cfg.misc.misctext.a()), HFONT_CENTERED_Y, watermark.c_str());
...
{
	// note: simv0l - change it later.
	//if (!g_cfg.misc.chat)
		return;

	static std::string chatspam[] = 
	{ 
		crypt_str("flipix - stay with us or lose the game."),
		crypt_str("get good. get flipix."),
		crypt_str("flipix - just the best."),
		crypt_str("go to another level with flipix."),
	};
...
void misc::hideradaronradar()
{
	static auto mat_radar = m_cvar()->FindVar(crypt_str("cl_drawhud_force_radar"));

	if (!g_cfg.misc.ingame_radar)
		mat_radar->SetValue(0);
	else if (g_cfg.misc.ingame_radar)
		mat_radar->SetValue(-1);
