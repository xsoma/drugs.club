...
{
	ImGui::StyleColorsClassic(); // colors setup 	ImGui::StyleColorsClassic
	ImGui::SetNextWindowSize(ImVec2(width, height), ImGuiCond_Once); // window pos setup
	//ImGui::SetNextWindowBgAlpha(min(style.Alpha,1.f)); // window bg alpha setup //0,94
...
std::string get_config_dir()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, 0x001a, NULL, NULL, path)))
		folder = std::string(path) + crypt_str("\\flipix\\Configs\\");
...
void add_config()
{
	...
	if (g_cfg.new_config_name.find(crypt_str(".dr")) == std::string::npos)
		g_cfg.new_config_name += crypt_str(".dr");
...
bool LabelClick(const char* label, bool* v, const char* unique_id)
{
	...
	const ImRect check_bb(window->DC.CursorPos, ImVec2(label_size.y + style.FramePadding.y * 2 + window->DC.CursorPos.x, window->DC.CursorPos.y + label_size.y + style.FramePadding.y * 2)); //FramePadding.x
	...
	if (label_size.x > 0)
	{
		...
		const ImRect text_bb(ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + style.FramePadding.y), ImVec2(window->DC.CursorPos.x + label_size.x, window->DC.CursorPos.y + style.FramePadding.y + label_size.y)); //y

		ImGui::ItemSize(ImVec2(text_bb.GetWidth(), check_bb.GetHeight()), style.FramePadding.y); //y FramePadding.x
...
void draw_semitabs(const char* labels[], int count, int& tab, ImGuiStyle& style)
{
   ...
   // ImGui::SetCursorPosX(0);
	...
	float sanyahuysosi = ImGui::GetCursorPosX();
	float sanyahuysosi1 = ImGui::GetCursorPosY();
	// center of main child

	
	// text size padding + frame padding
	//for (int i = 0; i < count; i++)
    	//sanyahuysosi -= (ImGui::CalcTextSize(labels[i]).x) /2 + style.FramePadding.x ; // + style.FramePadding.x * 2
	// offset = 30;
	// set new padding
	...
	for (int i = 0; i < count; i++)
	{
		...
		{
			//ImGui::SameLine();
		//	ImGui::SetCursorPosX(ImGui::GetCursorPosX() + style.ItemSpacing.x); //+ style.ItemSpacing.x
		
			//ImGui::SetCursorPosX(sanyahuysosi);
		}
	}
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\flipix\\Scripts\\");
...
void c_menu::draw_tabs_ragebot()
{
	...
	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth()/2,37+25), ImVec2(((ImGui::GetWindowWidth() / 2) - 150), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f,  1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f,   1.f)));

	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth() / 2, 37 + 25), ImVec2(((ImGui::GetWindowWidth() / 2) + 150), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)));


	ImGui::SetCursorPosX((ImGui::GetWindowWidth() - ImGui::CalcTextSize("General Weapon").x) / 2);
	const char* rage_sections[2] = { crypt_str("General"), crypt_str("Weapon") };
	auto p = ImGui::GetWindowPos();
	//ImGui::GetWindowDrawList()->AddLine(p, ImVec2(p.x + (700 * dpi_scale), p.y), IM_COL32(120, 0, 0, 255), 3.0f); //ImColor cRainbow IM_COL32(255, 0, 0, 255)
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				if (g_cfg.ragebot.double_tap)
				{
					...
					//	ImGui::SliderInt(crypt_str("Double tap override"), &g_cfg.ragebot.double_tab_override, 8, 16, false, crypt_str("%d°"));

						//ImGui::SliderInt(crypt_str("Double tap tolerance"), &g_cfg.ragebot.double_tab_tolerance, 0, 8, false, crypt_str("%d°"));
	...
	else
	{
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				{
					ImGui::SliderFloat(crypt_str("Head scale"), &g_cfg.ragebot.weapon[hooks::rage_weapon].head_scale, 0.f, 100.0f, g_cfg.ragebot.weapon[hooks::rage_weapon].head_scale ? crypt_str("%.2f") : crypt_str("None"));
					ImGui::SliderFloat(crypt_str("Body scale"), &g_cfg.ragebot.weapon[hooks::rage_weapon].body_scale, 0.f, 100.0f, g_cfg.ragebot.weapon[hooks::rage_weapon].body_scale ? crypt_str("%.2f") : crypt_str("None"));
				}
...
void c_menu::draw_legit(int child)
{
	...
	if (!child)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			float hue = staticHue + (1.f / (float)10) * i;
			if (hue < 0.f) hue += 1.f;
			ImColor cRainbow = ImColor::HSV(hue, 1.f, 1.f);
			ImGui::GetWindowDrawList()->AddLine(p, ImVec2(p.x + (700 * dpi_scale), p.y), IM_COL32(120, 0, 0, 255), 3.0f);

		}
	//	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(p, ImVec2(p.x + (700 * dpi_scale), p.y), ImGui::GetColorU32(ImVec4(120 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(120 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 255 / 255.f)));
...
void c_menu::draw_tabs_visuals()
{
	...
	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth() / 2, 37 + 25), ImVec2(((ImGui::GetWindowWidth() / 2) - 350), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)));

	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth() / 2, 37 + 25), ImVec2(((ImGui::GetWindowWidth() / 2) + 350), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)));


	ImGui::SetCursorPosX((ImGui::GetWindowWidth() - ImGui::CalcTextSize("General Viewmodel Skinchanger").x) / 2);
		...
		if (!child)
		{
			if (!visuals_section)
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					ImGui::Text(crypt_str("Color choke"));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##indcolorleft"), &g_cfg.esp.indcolorl, ALPHA);

					ImGui::Text(crypt_str("Color fake"));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##indcolorright"), &g_cfg.esp.indcolorr, ALPHA);
					...
					ImGui::Text(crypt_str("Fire 3D cycle color "));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##grenade_3dfire_color"), &g_cfg.esp.grenade_3dfire_color, ALPHA);

					//ImGui::Text(crypt_str("Fire 3D cycle color enemy "));
					//ImGui::SameLine();
					//ImGui::ColorEdit(crypt_str("##grenade_3dfire_enemy_color"), &g_cfg.esp.grenade_3dfire_color_enemy, ALPHA);
					...
					ImGui::Text(crypt_str("Smoke 3D cycle color "));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##grenade_3dsmoke_color"), &g_cfg.esp.grenade_3dsmoke_color, ALPHA);
			...
			else
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL2_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					ImGui::InputInt(crypt_str("Viewmodel roll"), &g_cfg.esp.viewmodel_roll, -180, 100);
				}
		...
		else
		{
			if (!visuals_section)
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					//ImGui::Checkbox(crypt_str("Full gray"), &g_cfg.esp.drawgray); //wait to fix =(
					/*
					……………………………....…………._¸„„„„_
					……………………....…………...„--~*'¯…….'\
					………….…....………………… („-~~--„¸_….,/ì'Ì
					...
					*/
					...
					ImGui::Checkbox(crypt_str("Props chams"), &g_cfg.esp.drugsmod);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##allscolor"), &g_cfg.esp.drugsmod_color, ALPHA);


					draw_combo(crypt_str("Props material"), g_cfg.esp.drugsmod_type, chamstype, ARRAYSIZE(chamstype));

					if (g_cfg.esp.drugsmod_type != 6)
					{
						ImGui::Checkbox(crypt_str("double material "), &g_cfg.esp.drugsmod_double_material);
						ImGui::SameLine();
						ImGui::ColorEdit(crypt_str("##drugsdoublematerial"), &g_cfg.esp.drugsmod_double_material_color, ALPHA);
					}
					...
					ImGui::Checkbox(crypt_str("Show model          "), &g_cfg.esp.drugsmod_orig_material);
			...
			else
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					ImGui::Checkbox(crypt_str("Wireframe arms "), &g_cfg.esp.arms_wire_material);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##armswirematerial"), &g_cfg.esp.arms_wire_material_color, ALPHA);

					ImGui::Checkbox(crypt_str("Show model"), &g_cfg.esp.arms_orig_material);
					...
					ImGui::Checkbox(crypt_str("Wireframe weapon "), &g_cfg.esp.weapon_wire_material);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##weaponwirematerial"), &g_cfg.esp.weapon_wire_material_color, ALPHA);



					ImGui::Checkbox(crypt_str("Show model "), &g_cfg.esp.weapon_orig_material);
					...
					ImGui::Spacing();
//sleeve
					ImGui::Checkbox(crypt_str("Sleeve chams"), &g_cfg.esp.sleeve_chams);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##sleevechamscolors"), &g_cfg.esp.sleeve_chams_chams_color, ALPHA);

					draw_combo(crypt_str("Sleeve chams material"), g_cfg.esp.sleeve_chams_type, chamstype, ARRAYSIZE(chamstype));

					if (g_cfg.esp.weapon_chams_type != 6)
					{
						ImGui::Checkbox(crypt_str("Double sleeve mat"), &g_cfg.esp.sleeve_double_material);
						ImGui::SameLine();
						ImGui::ColorEdit(crypt_str("##sleevedoublematerial"), &g_cfg.esp.sleeve_double_material_color, ALPHA);
					}
					ImGui::Checkbox(crypt_str("Wireframe sleeve  "), &g_cfg.esp.sleeve_wire_material);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##sleevewirematerial"), &g_cfg.esp.sleeve_wire_material_color, ALPHA);

					ImGui::Checkbox(crypt_str("Show model  "), &g_cfg.esp.sleeve_orig_material);
					

					ImGui::Checkbox(crypt_str("Animated material  "), &g_cfg.esp.sleeve_animated_material);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##sleeveanimatedmaterial"), &g_cfg.esp.sleeve_animated_material_color, ALPHA);
//sleeve end
				}
...
{
	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth() / 2, 37 + 25), ImVec2(((ImGui::GetWindowWidth() / 2) - 250), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)));

	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth() / 2, 37 + 25), ImVec2(((ImGui::GetWindowWidth() / 2) + 250), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)));


	ImGui::SetCursorPosX((ImGui::GetWindowWidth() - ImGui::CalcTextSize("Enemy Team Local").x) / 2);
...
void c_menu::draw_players(int child)
{
	...
	if (!child)
	{
		...
		tab_start();
		ImGui::BeginChild(crypt_str("##ESP1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale)); //BeginChild
		{
		...
		ImGui::BeginChild(crypt_str("##ESP2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			if (g_cfg.player.type[player].chams[PLAYER_CHAMS_VISIBLE] || player == LOCAL && g_cfg.player.local_chams_type) //-V648
			{
				if (player == LOCAL && g_cfg.player.local_chams_type)
				{
					...
					ImGui::Checkbox(crypt_str("Wireframe local "), &g_cfg.player.fake_wire_material);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##localwirematerial"), &g_cfg.player.fake_wire_material_color, ALPHA);

					ImGui::Checkbox(crypt_str("Show model    "), &g_cfg.player.player1_orig_material);
				...
				else
				{
					...
					ImGui::Checkbox(crypt_str("Wireframe player "), &g_cfg.player.type[player].wire_material);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##playerwirematerial"), &g_cfg.player.type[player].wire_material_color, ALPHA);
					 
					ImGui::Checkbox(crypt_str("Show model       "), &g_cfg.player.type[player].player_orig_material);
...
{
	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth() / 2, 37 + 25), ImVec2(((ImGui::GetWindowWidth() / 2) - 170), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)));

	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowWidth() / 2, 37 + 25), ImVec2(((ImGui::GetWindowWidth() / 2) + 170), 37), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)));



	ImGui::SetCursorPosX((ImGui::GetWindowWidth() - ImGui::CalcTextSize("Main Additives").x) / 2);
		...
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				if (g_cfg.antiaim.fakelag)
				{
					...
					ImGui::SliderInt(crypt_str("Limit"), &g_cfg.antiaim.fakelag_limit, 1, 16);
					...
					if (enabled_fakelag_triggers)
						ImGui::SliderInt(crypt_str("Triggers limit"), &g_cfg.antiaim.triggers_fakelag_limit, 1, 16);
				}
			}
			ImGui::EndChild();
			tab_end();
...
/*
				if (g_cfg.misc.spectators_list)
				{
					...
					ImGui::Begin("Spectator List", &g_cfg.misc.spectators_list, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
					{
					
						ImGui::SetWindowSize(ImVec2(200, 15));
						ImGui::Columns(2);
						ImGui::Separator();
...
*/
				...
				if (g_cfg.misc.events_to_log[EVENTLOG_HIT] || g_cfg.misc.events_to_log[EVENTLOG_ITEM_PURCHASES] || g_cfg.misc.events_to_log[EVENTLOG_BOMB])
				{
					...
					ImGui::Text(crypt_str("Color left"));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##logscolorleft"), &g_cfg.misc.logscolorl, ALPHA);

					ImGui::Text(crypt_str("Color right"));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##logscolorright"), &g_cfg.misc.logscolorr, ALPHA);
				}
				...
				if (g_cfg.menu.watermark)
				{
					ImGui::Text(crypt_str("Text color"));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##watercolortext"), &g_cfg.misc.misctext, ALPHA);

					ImGui::Text(crypt_str("Color left"));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##watercolorleft"), &g_cfg.misc.misccolorl, ALPHA);

					ImGui::Text(crypt_str("Color right"));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##watercolorright"), &g_cfg.misc.misccolorr, ALPHA);

					//		ImGui::SliderInt(crypt_str("Watermark alpha left"), &g_cfg.misc.alphal, 0, 255);
					//		ImGui::SliderInt(crypt_str("Watermark alpha right"), &g_cfg.misc.alphar, 0, 255);

					ImGui::Checkbox(crypt_str("Vertical gradient"), &g_cfg.misc.verticalgrad);
				}
		...
		ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			if (ImGui::CustomButton(crypt_str("Open scripts folder"), crypt_str("##LUAS__FOLDER"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
			{
				...
				auto get_dir = [&folder]() -> void
				{
					...
					if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
						folder = std::string(path) + crypt_str("\\flipix\\Scripts\\");
...
void c_menu::draw(bool is_open)
{
	...
	ImGui::Begin(crypt_str("flipix"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_MenuBar);
	{
		...
		/*	// set padding to draw { tabs , main functional area } / close group to draw last bottom child at x-0 pos
			ImGui::SetCursorPos(ImVec2(10 * dpi_scale, 13 * dpi_scale)); // *dpi_scale


			ImGui::BeginChild(crypt_str("##tabs_area"), ImVec2(124 , 509 )); // there will be tabs * dpi_scale 124 509
			{																			 // KEYWORD : TABS_FIND
		...
		ImGui::SetNextWindowBgAlpha(5.f);
		...
		{			
			...
			if (last_tab > 4)
				child_height = 471; //471
			else
				child_height = 471; //435
			...
			*/
			...
			ImGui::EndGroup();
			ImGui::PushFont(futura_smal_menu);
			ImGui::PopFont();
			...
			for (int i = 0; i < 10; i++)
			{
				...
				//		ImGui::GetWindowDrawList()->AddRectFilled(ImGui::GetCursorPos() + p - ImVec2(0, 5), ImGui::GetCursorPos() + p + ImVec2(710 * dpi_scale, 50), cRainbow, 7.f * dpi_scale, ImDrawCornerFlags_Bot); //m_alpha
			}
	...
	//		ImGui::GetWindowDrawList()->AddRectFilled(ImGui::GetCursorPos() + p - ImVec2(0, 5), ImGui::GetCursorPos() + p + ImVec2(710 * dpi_scale, 50), cRainbow, 7.f * dpi_scale, ImDrawCornerFlags_Bot); //m_alpha
		...
		ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImGui::GetCursorPos() + p - ImVec2(0, (dpi_scale*5)+4), ImGui::GetCursorPos() + p + ImVec2(710 * dpi_scale, 55), ImGui::GetColorU32(ImVec4(200 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(200 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f)), ImGui::GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 255 / 255.f)));



		ImGui::PushFont(vladfont);

		ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2((710 * dpi_scale - ImGui::CalcTextSize(crypt_str("flipix")).x) / 2, (16 * dpi_scale) - (ImGui::CalcTextSize(crypt_str("flipix")).y / 2)));

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, m_alpha);
		ImGui::Text(crypt_str("flipix"));
		ImGui::PopStyleVar();
		ImGui::PopStyleColor();
		ImGui::PopFont();
	...
	ImGui::PushFont(futura_smal_menu);
	ImGui::SetNextWindowBgAlpha(0.f);
	if (ImGui::BeginMainMenuBar()) //BeginMainMenuBar BeginMenu
	{
		//ImGui::SetCursorPos(ImVec2{ 0, 0 });
		//ImGui::SetCursorPos(ImVec2{ 10, -35 });
	//	ImGui::Text("DRUGS");
	  //ImGui::Image(logotype, ImVec2(150.0f, 50.0f));
	//	ImGui::SetCursorPos(ImVec2{ 75, -35 });
	//	ImGui::Text(".club");
		auto vlad = ImGui::CalcTextSize("RageLegitVisualsPlayersMiscSettingsPlayersScripts").x;
		ImGui::SetCursorPosX( (  (ImGui::GetWindowWidth() - vlad + (ImGui::GetStyle().ItemSpacing.x)*7) / 2) -50 );
		ImGui::SetCursorPosY(-15);
		if (ImGui::MainTab(crypt_str("Rage"), ico_menu, crypt_str(""), active_tab == 0, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 0;
		if (ImGui::MainTab(crypt_str("Legit"), ico_menu, crypt_str(""), active_tab == 1, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 1;
		if (ImGui::MainTab(crypt_str("Visuals"), ico_menu, crypt_str(""), active_tab == 2, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 2;
		if (ImGui::MainTab(crypt_str("Players"), ico_menu, crypt_str(""), active_tab == 3, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 3;
		if (ImGui::MainTab(crypt_str("Misc"), ico_menu, crypt_str(""), active_tab == 4, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 4;
		if (ImGui::MainTab(crypt_str("Settings"), ico_menu, crypt_str(""), active_tab == 5, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 5;
		if (ImGui::MainTab(crypt_str("List"), ico_bottom, crypt_str(""), active_tab == 6, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 6;
		if (ImGui::MainTab(crypt_str("Scripts"), ico_bottom, crypt_str(""), active_tab == 7, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 7;
	
			switch (last_tab % 9)
			{
			case 0: 
				draw_tabs_ragebot(); break;
			case 2:
				draw_tabs_visuals(); break;
			case 3:
				draw_tabs_players(); break;
			case 4:
				draw_tabs_misc(); break;
			default:
				break;
			}

		//	if (ImGui::MainTab(crypt_str("Radar"), ico_bottom, crypt_str(""), active_tab == 8, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 8;


		ImGui::EndMainMenuBar(); //EndMainMenuBar EndMenu
	}

	if (logotype == nullptr) D3DXCreateTextureFromFileInMemoryEx(device, &images::logo, sizeof(images::logo), 150, 50, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &logotype);
