...
bool __stdcall hooks::hooked_createmove(float smt, CUserCmd* m_pcmd)
{
	...
	g_ctx.globals.exploits = misc::get().double_tap_key && g_ctx.globals.weapon->can_double_tap() || misc::get().hide_shots_key;
	...
	if (g_cfg.ragebot.enable && !g_ctx.globals.weapon->is_non_aim() && engineprediction::get().backup_data.flags & FL_ONGROUND && g_ctx.local()->m_fFlags() & FL_ONGROUND)
		slowwalk::get().create_move(m_pcmd, 0.5f + 0.32f * (16 - m_clientstate()->iChokedCommands));

	if (g_ctx.local()->is_alive())
	{
		if (g_cfg.ragebot.enable && key_binds::get().get_key_bind_state(21) && engineprediction::get().backup_data.flags & FL_ONGROUND && g_ctx.local()->m_fFlags() & FL_ONGROUND)
		{
			*(float*)((uintptr_t)g_ctx.local()->get_animation_state() + 292) = 0;
		}
	}
	...
	util::movement_fix(g_ctx.globals.wish_angle, m_pcmd);
