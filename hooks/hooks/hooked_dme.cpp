...
void __stdcall hooks::hooked_dme(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* bone_to_world)
{
	...
	auto is_player = strstr(name, "models/player") && strstr(name, "models/player") && model_entity->is_alive() && (g_cfg.player.type[ENEMY].chams[PLAYER_CHAMS_VISIBLE] || g_cfg.player.type[TEAM].chams[PLAYER_CHAMS_VISIBLE] || g_cfg.player.type[LOCAL].chams[PLAYER_CHAMS_VISIBLE] || g_cfg.player.fake_chams_enable || g_cfg.player.backtrack_chams);
	auto is_weapon = strstr(name, "weapons/v_") && !strstr(name, "arms") && g_cfg.esp.weapon_chams; //weapons/v_
	auto is_arms = strstr(name, "arms") && !strstr(name, "sleeve") && g_cfg.esp.arms_chams;
	auto is_sleeve = strstr(name, "sleeve") && g_cfg.esp.sleeve_chams; //sleeve World
	...
	if (m_modelrender()->IsForcedMaterialOverride() && !is_weapon && !is_arms && !is_sleeve && !is_drugmod)
		return original_fn(m_modelrender(), ctx, state, info, bone_to_world);

	m_renderview()->SetColorModulation(1.0f, 1.0f, 1.0f); //-V807

	if (!is_player && !is_weapon && !is_arms && !is_sleeve && !is_drugmod)
		return original_fn(m_modelrender(), ctx, state, info, bone_to_world);
...
//	m_materialsystem()->FindMaterial(crypt_str("models/inventory_items/dogtags/dogtags_lightray"), nullptr),
	...
	if (is_player)
	{
		...
		if (type == ENEMY)
		{
			...
			auto wire_material = materials[1];
			if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
			{
				if (g_cfg.player.type[ENEMY].chams[PLAYER_CHAMS_VISIBLE] && g_cfg.player.type[ENEMY].chams[PLAYER_CHAMS_INVISIBLE])
				{
					...
					if (g_cfg.player.type[ENEMY].player_orig_material)
					{
						original_fn(m_modelrender(), ctx, state, info, bone_to_world);
						//	m_modelrender()->ForcedMaterialOverride(nullptr);
					}
					...
					if (g_cfg.player.type[ENEMY].wire_material)
					{
						alpha = (float)g_cfg.player.type[ENEMY].wire_material_color.a() / 255.0f;

						float wire_color[3] =
						{
						    g_cfg.player.type[ENEMY].wire_material_color[0] / 255.0f,
							g_cfg.player.type[ENEMY].wire_material_color[1] / 255.0f,
							g_cfg.player.type[ENEMY].wire_material_color[2] / 255.0f
						};

						m_renderview()->SetBlend(alpha * alpha_modifier);
						util::color_modulate(wire_color, wire_material);

						wire_material->IncrementReferenceCount();
						wire_material->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, true);

						m_modelrender()->ForcedMaterialOverride(wire_material);
						original_fn(m_modelrender(), ctx, state, info, bone_to_world);
						m_modelrender()->ForcedMaterialOverride(nullptr);
					}
		...
		else if (type == TEAM)
		{
			...
			if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
			{
				if (g_cfg.player.type[TEAM].chams[PLAYER_CHAMS_VISIBLE] && g_cfg.player.type[TEAM].chams[PLAYER_CHAMS_INVISIBLE])
				{
					...
					if (g_cfg.player.type[TEAM].player_orig_material)
					{
						original_fn(m_modelrender(), ctx, state, info, bone_to_world);
						//	m_modelrender()->ForcedMaterialOverride(nullptr);
					}
					...
					if (g_cfg.player.type[TEAM].wire_material)
					{
						alpha = (float)g_cfg.player.type[TEAM].wire_material_color.a() / 255.0f;

						float wire_color[3] =
						{
							g_cfg.player.type[TEAM].wire_material_color[0] / 255.0f,
							g_cfg.player.type[TEAM].wire_material_color[1] / 255.0f,
							g_cfg.player.type[TEAM].wire_material_color[2] / 255.0f
						};
		...
		else if (m_input()->m_fCameraInThirdPerson)
		{
			...
			if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
			{
				if (g_cfg.player.type[LOCAL].chams[PLAYER_CHAMS_VISIBLE] && g_cfg.player.type[LOCAL].chams[PLAYER_CHAMS_INVISIBLE])
				{
					...
					if (g_cfg.player.type[LOCAL].player_orig_material)
					{
						original_fn(m_modelrender(), ctx, state, info, bone_to_world);
						//	m_modelrender()->ForcedMaterialOverride(nullptr);
					}
					...
					if (g_cfg.player.type[LOCAL].wire_material)
					{
						alpha = (float)g_cfg.player.type[LOCAL].wire_material_color.a() / 255.0f;

						float wire_color[3] =
						{
							g_cfg.player.type[LOCAL].wire_material_color[0] / 255.0f,
							g_cfg.player.type[LOCAL].wire_material_color[1] / 255.0f,
							g_cfg.player.type[LOCAL].wire_material_color[2] / 255.0f
						};
			...
			if (g_cfg.player.fake_chams_enable)
			{
				...
				if (g_cfg.player.player1_orig_material)
				{
					original_fn(m_modelrender(), ctx, state, info, g_ctx.globals.fake_matrix);
					//	m_modelrender()->ForcedMaterialOverride(nullptr);
				}
				...
				if (g_cfg.player.fake_wire_material)
				{
					alpha = (float)g_cfg.player.fake_wire_material_color.a() / 255.0f;

					float wire_color[3] =
					{
						g_cfg.player.fake_wire_material_color[0] / 255.0f,
						g_cfg.player.fake_wire_material_color[1] / 255.0f,
						g_cfg.player.fake_wire_material_color[2] / 255.0f
					};
	...
	else if (is_weapon)
	{
		...
		if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
		{
			...
			if (g_cfg.esp.weapon_orig_material)
			{
			...
			if (g_cfg.esp.weapon_wire_material)
			{
				alpha = (float)g_cfg.esp.weapon_wire_material_color.a() / 255.0f;

				float wire_color[3] =
				{
					g_cfg.esp.weapon_wire_material_color[0] / 255.0f,
					g_cfg.esp.weapon_wire_material_color[1] / 255.0f,
					g_cfg.esp.weapon_wire_material_color[2] / 255.0f
				};
	...
	else if (is_arms)
	{
		...
		if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
		{
			...
			if (g_cfg.esp.arms_orig_material)
			{
				original_fn(m_modelrender(), ctx, state, info, bone_to_world);
				//	m_modelrender()->ForcedMaterialOverride(nullptr);
			}
			...
			if (g_cfg.esp.arms_wire_material)
			{
				alpha = (float)g_cfg.esp.arms_wire_material_color.a() / 255.0f;

				float wire_color[3] =
				{
					g_cfg.esp.arms_wire_material_color[0] / 255.0f,
					g_cfg.esp.arms_wire_material_color[1] / 255.0f,
					g_cfg.esp.arms_wire_material_color[2] / 255.0f
				};
	...
	{
	auto alpha = (float)g_cfg.esp.sleeve_chams_chams_color.a() / 255.0f;

	auto material = materials[g_cfg.esp.sleeve_chams_type];
	auto double_material = materials[6];
	auto wire_material = materials[1];
	if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
	{
		float sleeve_color[3] =
		{
			g_cfg.esp.sleeve_chams_chams_color[0] / 255.0f,
			g_cfg.esp.sleeve_chams_chams_color[1] / 255.0f,
			g_cfg.esp.sleeve_chams_chams_color[2] / 255.0f
		};

		m_renderview()->SetBlend(alpha);
		util::color_modulate(sleeve_color, material);
		...
		if (g_cfg.esp.sleeve_orig_material)
		{
		...
		if (g_cfg.esp.sleeve_animated_material)
		{
			auto animated_material = materials[9];

			if (animated_material)
			{
				auto alpha = (float)g_cfg.esp.sleeve_animated_material_color.a() / 255.0f;

				float animated_color[3] =
				{
					g_cfg.esp.sleeve_animated_material_color[0] / 255.0f,
					g_cfg.esp.sleeve_animated_material_color[1] / 255.0f,
					g_cfg.esp.sleeve_animated_material_color[2] / 255.0f
				};
		...
		if (g_cfg.esp.sleeve_double_material && g_cfg.esp.sleeve_chams_type != 6)
		{
			alpha = (float)g_cfg.esp.sleeve_double_material_color.a() / 255.0f;

			float double1_color[3] =
			{
				g_cfg.esp.sleeve_double_material_color[0] / 255.0f,
				g_cfg.esp.sleeve_double_material_color[1] / 255.0f,
				g_cfg.esp.sleeve_double_material_color[2] / 255.0f
			};

			m_renderview()->SetBlend(alpha);
			util::color_modulate(double1_color, double_material);
		...
		if (g_cfg.esp.sleeve_wire_material)
		{
			alpha = (float)g_cfg.esp.sleeve_wire_material_color.a() / 255.0f;

			float wire_color[3] =
			{
				g_cfg.esp.sleeve_wire_material_color[0] / 255.0f,
				g_cfg.esp.sleeve_wire_material_color[1] / 255.0f,
				g_cfg.esp.sleeve_wire_material_color[2] / 255.0f
			};
	...
	{
	auto alpha = (float)g_cfg.esp.drugsmod_color.a() / 255.0f;

	auto material = materials[g_cfg.esp.drugsmod_type];
	auto double_material = materials[6];
	auto wire_material = materials[1];
	if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
	{
		float drugs_color[3] =
		{
			g_cfg.esp.drugsmod_color[0] / 255.0f,
			g_cfg.esp.drugsmod_color[1] / 255.0f,
			g_cfg.esp.drugsmod_color[2] / 255.0f
		};

		m_renderview()->SetBlend(alpha);
		util::color_modulate(drugs_color, material);
		...
		if (g_cfg.esp.drugsmod_orig_material)
		{
		...
		if (g_cfg.esp.drugsmod_animated_material)
		{
			auto animated_material = materials[9];

			if (animated_material)
			{
				auto alpha = (float)g_cfg.esp.drugsmod_animated_material_color.a() / 255.0f;

				float animated_color[3] =
				{
					g_cfg.esp.drugsmod_animated_material_color[0] / 255.0f,
					g_cfg.esp.drugsmod_animated_material_color[1] / 255.0f,
					g_cfg.esp.drugsmod_animated_material_color[2] / 255.0f
				};
		...
		if (g_cfg.esp.drugsmod_double_material && g_cfg.esp.drugsmod_type != 6)
		{
			alpha = (float)g_cfg.esp.drugsmod_double_material_color.a() / 255.0f;

			float double1_color[3] =
			{
				g_cfg.esp.drugsmod_double_material_color[0] / 255.0f,
				g_cfg.esp.drugsmod_double_material_color[1] / 255.0f,
				g_cfg.esp.drugsmod_double_material_color[2] / 255.0f
			};
		...
		if (g_cfg.esp.drugsmod_wire_material)
		{
			alpha = (float)g_cfg.esp.drugsmod_wire_material_color.a() / 255.0f;

			float wire_color[3] =
			{
				g_cfg.esp.drugsmod_wire_material_color[0] / 255.0f,
				g_cfg.esp.drugsmod_wire_material_color[1] / 255.0f,
				g_cfg.esp.drugsmod_wire_material_color[2] / 255.0f
			};
