...
namespace hooks
{
	...
	void GUI_Init(IDirect3DDevice9* pDevice)
	{
		...
		// Menu fonts
		c_menu::get().futura_small = io.Fonts->AddFontFromMemoryTTF((void*)profont, sizeof(profont), 15.f, &m_config, ranges);
		c_menu::get().futura_smal_menu = io.Fonts->AddFontFromMemoryTTF((void*)profont, sizeof(profont), 25.f, &m_config, ranges);


		c_menu::get().futura = io.Fonts->AddFontFromMemoryTTF((void*)profont, sizeof(profont), 17.f, &m_config, ranges);
		c_menu::get().futura_large = io.Fonts->AddFontFromMemoryTTF((void*)profont, sizeof(profont), 20.f, &m_config, ranges); //futuramediumc
		c_menu::get().gotham = io.Fonts->AddFontFromMemoryTTF((void*)GothamPro, sizeof(GothamPro), 13.f, &m_config, ranges);
		c_menu::get().vladfont = io.Fonts->AddFontFromMemoryTTF((void*)visitor_tt2_brk, sizeof(visitor_tt2_brk), 23.f, &m_config, ranges);
